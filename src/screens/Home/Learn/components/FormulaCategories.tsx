...
import FormulaChapter from './FormulaChapter'

type FormulaCategoriesProps = {
  categories?: ChaptersCategory[]
}
const FormulaCategories: FC<FormulaCategoriesProps> = ({ categories }) => {
            ...
            {!isCollapsed && (
              <View className='flex-row flex-wrap gap-2.5 px-5'>
                {chapters.map((chapter) => (
                  <FormulaChapter key={chapter.chapter} chapter={chapter} />
                ))}
              </View>
            )}
          </Animated.View>
